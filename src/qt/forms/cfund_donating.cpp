#include "cfund_donating.h"

#include "txdb.h"
#include "consensus/cfund.h"
#include "main.h"

CFund_Donating::CFund_Donating(QWidget *parent, bool fPaymentRequests) :
    QDialog(parent),
    ui(new Ui::CFund_Donating),
...
void CFund_Donating::closeDialog() {
    close();
}

void CFund_Donating::voteYes() {
    if (selected == "")
        return;
    bool d = false;
    if (!fSettings) {
...
void CFund_Donating::stopVoting() {
    if (selected == "")
        return;
    if (!fSettings) {
...
void CFund_Donating::viewDetails() {
    if (selected == "")
        return;
...
void CFund_Donating::switchView() {
    setSelection("");
    fSettings = !fSettings;
    Refresh();
}

void CFund_Donating::selectedFromYes(QListWidgetItem* item) {
...
void CFund_Donating::selectedFromNo(QListWidgetItem* item) {
   ui->notvotingList->clearSelection();
...
void CFund_Donating::selectedFromNotVoting(QListWidgetItem* item) {
    ui->votingnoList->clearSelection();
    ui->votingyesList->clearSelection();
...
void CFund_Donating::setSelection(QString selection) {
    selected = selection;
    enableDisableButtons();
}

void CFund_Donating::enableDisableButtons() {
